class ProductsController < ApplicationController
  ...
  def create
	  ...
	  redirect_to root_path, notice: "Success! Product is created."
  end

  def upload
	  uploaded_pics = params[:file] # Take the files which are sent by HTTP POST request.
	  time_footprint = Time.now.to_formatted_s(:number) # Generate a unique number to rename the files to prevent duplication
